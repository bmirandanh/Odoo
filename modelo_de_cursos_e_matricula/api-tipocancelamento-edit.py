...
update_tipocancelamento_url = f"{odooserver_url}/api/tipocancelamento/update/{cod_cancelamento}" + (f"/{attribute}" if attribute else "")
...
if response_login.status_code == 200 and response_login.json().get("result"):
    ...
    response_update = session.put(update_tipocancelamento_url, headers=headers, data=json.dumps(update_data))  # Usando data= ao invés de json=
